...
class MazeSolver:
    def __init__(self):
        ...
        # 控制参数
        self.speed_scale = float(rospy.get_param('~speed_scale', 2.2))
        self.safe_distance = 0.45
        self.max_linear = 0.30 * self.speed_scale
        self.max_angular = 1.2 * self.speed_scale
        ...
        self.wall_dist = 0.45
        self.wall_kp = 1.5
        self.wall_linear = 0.18 * self.speed_scale
        ...
        self.min_frontier_goal_dist = float(rospy.get_param('~min_frontier_goal_dist', 0.45))
        ...
        # 聚类参数（任务二：按颜色分别做空间聚类）
        self.cluster_radius = float(rospy.get_param('~cluster_radius', 1.00))
        self.cluster_merge_radius = float(rospy.get_param('~cluster_merge_radius', 1.30))
        self.cluster_max_span = float(rospy.get_param('~cluster_max_span', 1.05))

        # 新目标确认机制：连续命中后再入库，减少颜色误检导致的假目标
        ...
        self.pending_merge_radius = float(rospy.get_param('~pending_merge_radius', 0.55))
    ...
    def _register_target(self, color_name, x, y, px, py, area):
        ...
        for item in target_list:
            if math.hypot(item['x'] - x, item['y'] - y) < self.detection_merge_radius:
                ...
                self._append_detection_event(color_name, item['x'], item['y'], px, py, area, now)
                return False
        ...
        self._append_detection_event(color_name, x, y, px, py, area, now)
        rospy.loginfo("Target detected! Location: (%.2f, %.2f, %.2f) color=%s", x, y, self.target_z, color_name)
        return True

    def _append_detection_event(self, color_name, x, y, px, py, area, ts):
    ...
    def _build_target_report(self):
        ...
        return {
            ...
            'all_detection_summary': {
                'green': len(self.detection_events['green']),
                'red': len(self.detection_events['red']),
                'yellow': len(self.detection_events['yellow']),
                'blue': len(self.detection_events['blue']),
                'total': len(self.detection_events['green']) + len(self.detection_events['red']) +
                         len(self.detection_events['yellow']) + len(self.detection_events['blue']),
            },
    ...
    def _save_markdown_report(self, report):
        try:
            ...
            for c in clusters:
                ...
                for i, m in enumerate(all_events, 1):
                    lines.append('  - %d) (%.2f, %.2f, %.2f)' % (
                        i,
                        float(m.get('x', 0.0)),
                        float(m.get('y', 0.0)),
                ...
                for j in range(len(points)):
                    ...
                    if math.hypot(x1 - x2, y1 - y2) <= self.cluster_radius:
                        if group:
                            ...
                            if math.hypot(x2 - cxg, y2 - cyg) > (self.cluster_radius + 0.35):
                                continue
                        visited[j] = True
                        q.append(j)
